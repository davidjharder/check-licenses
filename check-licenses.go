// Checks the license key(s) of all package.yml files against the list of SPDX identifiers.
...
const licenseListJSONfile = "common/Go/check-licenses/licenses.json"

// type to hold parsed license JSON data
...
// types to hole the parsed yaml data, where license keys come in two flavors
...
func main() {
	...
	// Walk the "packages" directory, passing any found package.yml files off to the checker
	err = filepath.Walk("packages", func(path string, info fs.FileInfo, err error) error {
		if err != nil {
			fmt.Println("File walk error at path: ", path)
			return err
		}
		//fmt.Println(path)
...
// Yaml file is parsed twice because the license key can either contain a single string or a list of strings
func licenseCheck(yamlPath string, licenseList []string) {
	YAMLfileContent, err := os.ReadFile(yamlPath)
	if err != nil {
		log.Fatal("Error when opening file: ", yamlPath, err)
	}
	var yamlParseString yamlDataString
	var yamlParseSlice yamlDataSlice

	// Try parsing the yaml file with a single string at the license key
	...
	if err != nil {
		...
		return // return early for yaml files with license *list*
	}
